...
cle_tracts = pd.read_csv('cuyahoga_tracts.csv')
cle_tracts = cle_tracts[cle_tracts['placenme'] == 'Cleveland city']

race = pd.read_csv('white_pct.csv', skiprows=3)
race['pct_nonwhite'] = 100-race['pct_white']
race['pct_nonwhite'] = race['pct_nonwhite'].map(lambda x: max(0, x))
...
lead_levels = pd.merge(lead_levels, race, on='Census Tract')
# lead_levels = lead_levels[lead_levels['Census Tract'].isin(cle_tracts['tract'])]
print(lead_levels.head())

map_df = gpd.read_file("tl_2015_39_tract.shp")
# print(map_df.crs)
tract_ids = lead_levels['Census Tract']*100
...
print(map_df['GEOID'])
fips = [str(g)[:-1] for g in cle_tracts['stfid00_bg']]
map_df = map_df[map_df['GEOID'].isin(fips)]
...
ax2.set_title('Percent of population who are not white')
